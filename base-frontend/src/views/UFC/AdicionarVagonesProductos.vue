          ...
          <div class="col-md-6">
            ...
            <div class="mb-3">
              <label for="plan_anual" class="form-label"
                >Plan anual <span style="color: red" v-if="esPlanAnualEditable">*</span></label
              >
              <input
                type="number"
                class="form-control"
  ...
  methods: {
    ...
    async submitForm() {
      try {
          ...
          this.formData.lista_productos = this.formData.lista_productos.map(id => parseInt(id));

          if (productosParaEnviar.length === 0 && this.formData.lista_productos.length > 0) {
              Swal.fire("Error", "Los productos seleccionados no son válidos", "error");
              return;
          }

          // Validar que los productos seleccionados existan
          const productosValidos = await this.validarProductos(this.formData.lista_productos);
          if (!productosValidos) {
