...
export default createStore({
  ...
  mutations: {
    initializeStore(state) {
      if (localStorage.getItem('token')) {/*aqui estamos preguntando si se tiene almacenado el token en el navegador */
      ...
      } else { /*en caso de no estar autenticado poner la variable del token a vacio, el estado autenticado a falso */
        state.token = ''
    ...
    setToken(state, token) {/*funcion para modificar el token, cuando se está autenticado */
      state.token = token
      state.isAuthenticated = true
    },
    removeToken(state) { /*funcion para eliminar el token, se pone en falso la autenticacion, estas funciones seran usadas al abrir la app
