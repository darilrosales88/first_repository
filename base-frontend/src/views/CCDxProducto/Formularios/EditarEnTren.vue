<template>
    ...
    <div class="card border">  
      <div class="card-header bg-light border-bottom">
        <h5 class="mb-0 text-dark fw-semibold">
          <i class="bi bi-file-earmark-plus me-2"></i> Nuevo registro de vagón en tren
        </h5>
      </div>
        ...
        <form @submit.prevent="submitForm">
          <div class="row">
            <div class="col-md-6">
              ...
              <div class="mb-3">
                <label for="cantidad_vagones" v-if="equiposList.length == 0" class="form-label small fw-semibold text-secondary">Deshabilitado - No existen vagones</label>
              ...
              <div class="mb-3">
                <label for="equipo_ferroviario" class="form-label small fw-semibold text-secondary">Equipo Ferroviario</label>
                <select 
                    v-if="equiposList.length > 0"
                    class="form-select form-select-sm border-secondary"
                    style="padding: 8px 12px"
            ...
            <div class="mb-3">
              <label for="causas" class="form-label small fw-semibold text-secondary">Observaciones</label>
              <textarea
                class="form-control form-control-sm border-secondary"
  ...
  <div class="container py-3" style="margin-left: 20em; width: 70%">
    <div class="card border">
      ...
      <div class="card-body p-3">
        <div class="d-flex justify-content-between align-items-center mb-4">
          <button class="btn btn-primary" @click="annadirNuevoVagon()">
...
export default {
  ...
  methods: {
    ...
    async get_registro() {
      try {
        const response = await axios.get(`/ufc/ccd-en-trenes/${this.registroId}/`);
        ...
        this.formData.observaciones = registro.observaciones
        
        for(let i = 0; i < registro.equipo_vagon_detalle.length; i++) {
          let vagon = {
              equipo_ferroviario: registro.equipo_vagon_detalle[i],
          };
          this.vagonesAnnadidos.push(vagon);
        }       
    ...
    eliminarVagon(item,index) {
        this.equiposList.push(item.equipo_ferroviario)
    ...
    async submitForm() {
      try {
        ...
        const payload = {
          ...
          equipo_vagon: this.vagonesAnnadidos.map((vagon) => (vagon.equipo_ferroviario.id)),
        };
        
        const response = await axios.patch(`/ufc/ccd-en-trenes/${this.registroId}/`, payload);
        this.resetForm();
        this.$router.push({ name: "ccdxproducto" });
        this.showSuccessToast("Registro editado");
      } catch (error) {
        if(error.response.status===400){
          this.showErrorToast("Los campos producto, tipo de equipo y acceso deben formar un conjunto único");
        }else{
