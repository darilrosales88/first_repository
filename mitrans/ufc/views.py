...
class vagon_cargado_descargado_view_set(viewsets.ModelViewSet):
    ...
    def obtener_registros_completos(self, request, pk=None):
        instance = self.get_object()
        registros = instance.registros_vagones.all()
        serializer = registro_vagones_cargados_serializer(registros, many=True)
        return Response(serializer.data)
    ...
    @action(detail=True, methods=['get'], url_path='vagones')
    def listar_vagones_asociados(self, request, pk=None):
        """
        Endpoint: /api/ufc/vagones-cargados-descargados/<id>/vagones/
        """
        instance = self.get_object()
        vagones = instance.registros_vagones.all()
        serializer = registro_vagones_cargados_serializer(vagones, many=True)
        return Response(serializer.data)
