          ...
          <form @submit.prevent="search_producto" class="search-container">
            <div class="input-group">
              <input
                type="search"
                class="form-control"
                placeholder="Buscar en los partes"
                v-model="searchQuery"
        ...
        <div class="d-flex justify-content-between align-items-center mb-4">
          <table class="table table-sm table-bordered table-hover">
              ...
              <tr v-if="filteredRecords.length == 0">
                <td colspan="9" class="text-center text-muted py-4">
                  ...
                  <div v-else>
                    ...
                    <p class="mt-2">
                      {{
                        searchQuery ? "No hay coincidencias" : "No hay partes registrados"
                      }}
                    </p>
                    <p>
                      {{
                        searchQuery
                          ...
                          : "No hay partes de hechos extraordinarios registrados"
                      }}
                    </p>
                  </div>
                </td>
            ...
            <tbody>
              <!-- Filas de datos -->
              <tr
                v-for="(parte, index) in filteredRecords"
                :key="parte.id"
                class="io-tr"
              >
                ...
                <td>
                  <span
                    ...
                    >{{ parte.estado_parte || "-" }}</span
                  >
                </td>
                <td>{{ parte.creado_por_detalle || "-" }}</td>
                <td>
                  {{
                    parte.aprobado_por_detalle
                      ? parte.aprobado_por_detalle
                      : "-"
                  }}
                </td>
                ...
                <td>
                  <div class="d-flex">
                    ...
                    <button
                      @click="confirmDelete(parte.id)"
                      class="btn btn-sm btn-outline-danger"
                      title="Eliminar"
                      :disabled="loading || parte.estado_parte === 'Aprobado'"
                    >
...
export default {
  ...
  computed: {
    filteredRecords() {
      if (!this.searchQuery) return this.partes;
      ...
      return this.partes.filter((parte) => {
        const fieldsToSearch = [
          ...
          parte.creado_por_detalle?.username,
          parte.aprobado_por_detalle?.username,
        ];
  ...
  methods: {
    ...
    async getPartes() {
      ...
      try {
        ...
        console.log("estos son los partes: ",response.data)
        this.partes = response.data.results.map(parte => ({
          ...parte,
          entidad_detalle: parte.entidad_name,
          provincia_detalle: parte.provincia_name,
          creado_por_detalle: parte.creado_por_name,
          aprobado_por_detalle: parte.aprobado_por_name
        }));
        this.totalItems = response.data.count;
      } catch (error) {
        console.error("Error al obtener los partes:", error);
        this.showErrorToast("No se pudieron cargar los partes");
      } finally {
        this.loading = false;
      }
    },
    ...
    async searchPartes() {
      ...
      try {
        ...
        this.partes = response.data.results.map(parte => ({
          ...parte,
          entidad_detalle: parte.entidad,
          provincia_detalle: parte.provincia,
          creado_por_detalle: parte.creado_por,
          aprobado_por_detalle: parte.aprobado_por
        }));
        this.totalItems = response.data.count;
      } catch (error) {
    ...
    async deleteParte(id) {
      try {
        await axios.delete(`/api/gemar-partes-hechos-extraordinarios/${id}/`);
        ...
        this.showErrorToast("Error al eliminar el parte");
      }
    },
