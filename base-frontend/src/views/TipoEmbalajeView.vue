      ...
      <div class="card border">
        <div class="card-header bg-light border-bottom">
          <h6 class="mb-0 text-dark fw-semibold">
            Listado de tipos de embalajes
          </h6>
        </div>
        <div class="card-body p-3">
          <div class="d-flex justify-content-between align-items-center mb-4">
            <router-link v-if="hasGroup('Admin')" to="AdicionarTipoEmbalaje">
              <button class="btn btn-sm btn-primary">
                <i class="bi bi-plus-circle me-1"></i>Adicionar tipo de embalaje
              </button>
            </router-link>
            <form @submit.prevent="searchEmbalajes" class="search-container">
              <div class="input-group">
                <input
                  type="search"
                  class="form-control"
                  placeholder="Buscar por tipo de embalaje"
                  v-model="searchQuery"
                ...
                <tr v-for="item in embalajes" :key="item.id">
                  ...
                  <td>
                    <div class="d-flex" style="margin-left: 30%;">
                      <button 
                        @click="openTipoEmbalajeDetailsModal(item)"
  ...
  methods: {
    ...
    async get_embalajes() {
      ...
      try {
        ...
        this.embalajes = response.data.results; // Obtener los registros de la página actual
        ...
        this.busqueda_existente = true; // Reinicia la variable al cargar todos los embalajes
      } catch (error) {
        console.error("Error al obtener los embalajes:", error);
        Swal.fire("Error", "No se pudieron cargar los embalajes.", "error");
      }
    ...
    async deleteEmbalaje(id) {
      try {
        ...
        this.embalajes = this.embalajes.filter(
          (embalaje) => embalaje.id !== id
        );
        Swal.fire(
          "Eliminado!",
          "El tipo de embalaje ha sido eliminado exitosamente.",
          "success"
        );
      } catch (error) {
        ...
        Swal.fire(
          "Error",
          "Hubo un error al eliminar el tipo de embalaje.",
          "error"
        );
      }
    },
    openTipoEmbalajeDetailsModal(TipoEmbalaje) {
      ...
      const gruposAsignados =
        TipoEmbalaje.groups && TipoEmbalaje.groups.length > 0
          ? TipoEmbalaje.groups
              .map((groupId) => {
      ...
      const permisosAsignados =
        ...
        TipoEmbalaje.TipoEmbalaje_permissions.length > 0
          ? TipoEmbalaje.TipoEmbalaje_permissions.map((permisoId) => {
