  ...
  <div class="container">
    ...
    <div v-if="showModal" class="modal-backdrop">
      ...
      <div class="mb-3">
        ...
        <input type="text" class="form-control" id="situados" v-model="nuevoRegistro.cantidad_por_situar" required />
      </div>
...
getPorSituar() {
  ...
  try {
    ...
    const camposRequeridos = [
      ...
      { nombre: 'cantidad_por_situar', valor: this.nuevoRegistro.cantidad_por_situar }
    ];
    ...
    const response = await axios.post('http://127.0.0.1:8000/api/por_situar/', datosParaEnviar);
  ...
  search_por_situar() {
    this.loading = true;
    axios.get(`http://127.0.0.1:8000/api/por_situar/?tipo_equipo=${this.searchQuery}`)
      .then((response) => {
  ...
  async confirmDelete(id) {
    ...
    async deleteItem(id) {
      this.loading = true;
      try {
        const response = await axios.delete(`http://127.0.0.1:8000/api/por_situar/${id}/`);
