...
export default {
  ...
  methods: {
    ...
    getPorSituar() {
      this.loading = true;
      axios
        .get("http://127.0.0.1:8000/ufc/situados/")
        .then((response) => {
    ...
    async addNewItem() {
      try {
        ...
        const camposRequeridos = [
          ...
          {
            nombre: "pendiente_proximo_dia",
            valor: this.nuevoRegistro.pendiente_proximo_dia,
          },
        ];
    ...
    search_por_situar() {
      ...
      axios
        .get(
          `http://127.0.0.1:8000/ufc/situados/?tipo_equipo=${this.searchQuery}`
        )
        .then((response) => {
