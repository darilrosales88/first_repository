        ...
        <div class="card border">
            <div class="card-header bg-light border-bottom">
                <h6 class="mb-0 text-dark fw-semibold">
                    <i class="bi bi-train-front-fill me-2"></i>Vagones Pendientes de Arrastre
                </h6>
            </div>
            <div class="card-body p-3">
                <div class="d-flex justify-content-between align-items-center">
                    <router-link v-if="hasGroup(['AdminUFC', 'OperadorUFC'])" to="/AnnadirPendienteArrastreCCD">
...
export default {
  ...
  methods: {
    ...
    async getElements() {
      try {
        ...
        if (this.informeID || infoID.data.id) {
            this.loading = true;
            const response = await axios.get("/ufc/ccd-arrastres/", {
                params: {
    ...
    async delete_element(id) {
      try {
        await axios.delete(`/ufc/ccd-arrastres/${id}/`);
