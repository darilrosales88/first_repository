      ...
      <div class="card border">
        <div class="card-header bg-light border-bottom">
          <h6 class="mb-0 text-dark fw-semibold">
            <i class="bi bi-clipboard-data me-2"></i>Registro de programación de maniobras asociados
          </h6>
        </div>
          ...
          <form @submit.prevent="submitForm">
              ...
              <div class="d-flex justify-content-end gap-2 mt-4">
                <button
                  ...
                  @click="crearParteProgramacionManiobra"
                  :disabled="isExistingRecord || loading"
                >
                  <span v-if="loading" class="spinner-border spinner-border-sm" role="status" aria-hidden="true"></span>
                  <i v-else class="bi bi-save me-2"></i>
                  {{ loading ? 'Creando...' : 'Crear parte de programación de maniobras' }}
                </button>
              </div>
            </form>
        </div>
...
export default {
  name: "ProgramacionManiobrasView",
  components: {
    NavbarComponent,
  ...
  data() {
    ...
    return {
      ...
      userPermissions: [],
      userGroups: [],
      currentComponent: "programacion_maniobra",
      informeParteId: null,
  ...
  async created() {
    ...
    await this.verificarParteProgramacionManiobras();
  },

  methods: {
    async verificarParteProgramacionManiobras() {
    ...
    async crearParteProgramacionManiobra() {
        this.loading = true;
        this.error = null;
        this.success = false;

        try {
            // Verificar si ya existe un informe de programacion de maniobra para la fecha actual
            const verificacion = await axios.get(
                "/gemar/gemar-verificar-informe-programacion-maniobra-existente/",
                {
                    params: {
            ...
            const response = await axios.post(
                "/gemar/gemar-partes-programacion-maniobras/",
                datosEnvio
            );
            ...
            await Swal.fire({
                icon: "success",
                title: "Éxito",
                text: "Parte de programación de maniobras creado correctamente",
                confirmButtonColor: "#002a68",
            });
    ...
    async rechazar() {
      if (!this.hasGroup("RevisorGEMAR")) {
        await Swal.fire({
          ...
          text: "No tienes permiso para rechazar el parte de programación de maniobras.",
          confirmButtonColor: "#002a68",
        });
        return;
      }
      const result = await Swal.fire({
        title: "¿Estás seguro?",
        text: "Está seguro que desea rechazar  el parte de programación de maniobras?",
        icon: "question",
    ...
    async aprobar() {
      if (!this.hasGroup("RevisorGEMAR")) {
        await Swal.fire({
          icon: "error",
          title: "Acceso denegado",
          text: "No tienes permiso para aprobar  el parte de programación de maniobras.",
          confirmButtonColor: "#002a68",
        });
        return;
      }
      ...
      const result = await Swal.fire({
        title: "¿Estás seguro?",
        text: "Está seguro que desea aprobar  el parte de programación de maniobras?",
        icon: "question",
        showCancelButton: true,
        confirmButtonColor: "#002a68",
        cancelButtonColor: "#d33",
    ...
    async listo() {
      if (!this.hasGroup("AdminGEMAR")) {
        await Swal.fire({
          icon: "error",
          title: "Acceso denegado",
          text: "No tienes permiso para cambiar el estado  del parte de programación de maniobras a Listo.",
          confirmButtonColor: "#002a68",
        });
        return;
      }
      const result = await Swal.fire({
        title: "¿Estás seguro?",
        text: "Está seguro que desea poner a 'Listo' este parte de programación de maniobras?",
        icon: "question",
        showCancelButton: true,
        confirmButtonColor: "#002a68",
        cancelButtonColor: "#d33",
    ...
    async CambiarEstado(NuevoEstado) {
      try {
        ...
        const response = await axios.patch(
          `/gemar/gemar-partes-programacion-maniobras/${this.informeParteId}/`,
