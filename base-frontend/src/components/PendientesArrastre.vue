                  ...
                  <p>
                    {{
                      searchQuery
                        ...
                        : "No hay vagones pendientes de arrastre"
                    }}
                  </p>
                  <router-link
...
export default {
  ...
  computed: {
    filteredRecords() {
      ...
      return this.registrosArrastre.filter((item) => {
        const fieldsToSearch = [
          this.getTipoOrigenText(item.tipo_origen),
          item.origen,
          item.tipo_equipo,
          this.getEstadoText(item.estado),
          item.productos_info ? this.getNombresProductos(item.productos_info) : '',
          item.cantidad_vagones,
          item.destino,
          item.observaciones,
        ];
  ...
  methods: {
    ...
    getStatusClass(status) {
      ...
      if (statusLower.includes("vacio")) return "secondary";
    ...
    async confirmDelete(id) {
      ...
      if (result.isConfirmed) {
        try {
          this.loading = true;
          await axios.delete(`http://127.0.0.1:8000/ufc/pendiente-arrastre/${id}/`);
