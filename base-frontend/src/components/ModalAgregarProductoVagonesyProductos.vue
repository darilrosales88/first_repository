<template>
  ...
  export default {
    ...
    methods: {
        ...
        try {
            const productoExiste = await this.verificarProducto(this.formData.producto);
            if (!productoExiste) {
            Swal.fire("Error", "El producto seleccionado no existe", "error");
            return;
            }

            const response = await axios.post("/ufc/productos-vagones-productos/", {
            ...this.formData,
            producto: parseInt(this.formData.producto) // Asegurar que es número
            });

            Swal.fire("Éxito", "Producto creado correctamente", "success");
            this.$emit("cerrar-modal");
            this.$emit("actualizar-productos"); // Emitir evento para actualizar lista
        } catch (error) {
