    ...
    <div class="container py-3" style="margin-left: 17.3em; width: 75%">
      <div class="card border">
        <div class="card-header bg-light border-bottom">
          <h6 class="mb-0 text-dark fw-semibold">Listado de cargos</h6>
        </div>
        <div class="card-body p-3">
          <div class="d-flex justify-content-between align-items-center mb-4">
            <router-link v-if="hasGroup('Admin')" to="/AdicionarCargo">
              <button class="btn btn-sm btn-primary">
                <i class="bi bi-plus-circle me-1"></i>Adicionar cargo
              </button>
            </router-link>
            <form @submit.prevent="searchCargos" class="search-container">
...
export default {
  ...
  methods: {
    ...
    async searchCargos() {
      ...
      this.currentPage = 1;
      axios
        .get(`/api/cargos/?n_cargo=${this.searchQuery}&page=${this.currentPage}`)
        .then((response) => {
          this.cargos = response.data.results;
          this.totalPages = Math.ceil(response.data.count / 15);
          this.updatePages();
