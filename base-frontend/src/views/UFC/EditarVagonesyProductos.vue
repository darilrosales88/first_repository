<template>
          ...
          <div class="row">
            ...
            <div class="col-md-6">
              ...
              <div class="mb-3">
                ...
                required
                >
                <option value="" disabled>Seleccione un equipo ferroviario</option>
                <option
                    v-for="tipo in tipos_equipos_ferroviarios"
                    :key="tipo.id"
                    :value="tipo.id"
                >
                    {{ tipo.id }}-{{ tipo.tipo_equipo_name }} - {{ tipo.descripcion }}
                </option>
                </select>
            ...
            <div class="col-md-6">
              ...
              <div class="mb-3">
                ...
                >
                    ...
                    :value="producto.id"
                    >
                    {{ producto.id }}-{{ producto.producto_name || 'Nombre no disponible' }}
                    </option>
                </select>
  ...
  export default {
    ...
    async mounted() {
        ...
        await this.getProductos(); // Primero cargar todos los productos
        await this.getNoLocomotoras();
        await this.getEntidades();
        await this.getPuertos();
        await this.loadVagonData(); // Luego cargar los datos del registro
    },
    async created() {
      await this.fetchUserPermissionsAndGroups();
    },
    ...
    try {
        ...
        // 4. MANEJO ESPECÍFICO PARA PRODUCTOS - VERSIÓN DEFINITIVA
        let productosAsociados = [];
        let productosOriginales = [];

        // Caso 1: producto es un array de IDs (como [3] en tu respuesta)
        if (Array.isArray(response.data.producto)) {
            // Extraer IDs numéricos y convertirlos a strings
            productosAsociados = response.data.producto
                .filter(id => id != null && !isNaN(id))
                .map(id => id.toString());
            
            // Buscar los objetos completos en los productos disponibles
            productosOriginales = this.productos.filter(p => 
                productosAsociados.includes(p.id.toString())
            );
        }
        // Caso 2: producto es un array de objetos (formato alternativo)
        ...
        // Caso 4: producto es un ID directo (no común)
        ...
        console.log('IDS A SELECCIONAR:', productosAsociados);
        console.log('PRODUCTOS ORIGINALES:', productosOriginales);

        // 5. Establecer datos del formulario
        this.formData = {
        ...
        // 6. Forzar actualización del select multiple
        this.$nextTick(() => {
            // Asignar nuevo array para asegurar reactividad
            ...
            console.log('SELECT VALUE:', 
                document.querySelector('select[name="lista_productos"]')?.value);
        });
      ...
      async submitForm() {
        ...
        try {
          ...
          // 2. Validar que los productos seleccionados existan
          ...
          // 4. Enviar la solicitud PUT al backend
          const vagonId = this.$route.params.id;
