      ...
      <table class="table">
        ...
        <tbody>
          <tr v-for="item in tipo_maniobras" :key="item.id">
            ...
            <td>
              <button
                @click="openTipoManiobraDetailsModal(item)"
              ...
              <span v-if="hasGroup('Admin')">
                <button class="btn btn-warning btn-small">
                  <router-link
                    :to="{ name: 'EditarManiobra', params: { id: item.id } }"
                  >
  ...
  methods: {
    ...
    async getTipoManiobra() {
      try {
        ...
        this.tipo_maniobras = response.data.results; // Obtener los registros de la página actual
        ...
        this.busqueda_existente = true; // Reinicia la variable al cargar todos los tipos de maniobras
      } catch (error) {
        console.error("Error al obtener los tipos de maniobras:", error);
        Swal.fire(
          "Error",
          "No se pudieron cargar los tipos de maniobras.",
          "error"
        );
      }
    },
    ...
    async deleteTManiobra(id) {
      try {
        ...
        this.tipo_maniobras = this.tipo_maniobras.filter(
          (tmaniobra) => tmaniobra.id !== id
        );
        Swal.fire(
          "Eliminado!",
          "El tipo de maniobra ha sido eliminado exitosamente.",
          "success"
        );
      } catch (error) {
        ...
        Swal.fire(
          "Error",
          "Hubo un error al eliminar el tipo de maniobra.",
          "error"
        );
      }
    },
    openTipoManiobraDetailsModal(TipoManiobra) {
      ...
      const gruposAsignados =
        TipoManiobra.groups && TipoManiobra.groups.length > 0
          ? TipoManiobra.groups
              .map((groupId) => {
      ...
      const permisosAsignados =
        ...
        TipoManiobra.TipoManiobra_permissions.length > 0
          ? TipoManiobra.TipoManiobra_permissions.map((permisoId) => {
