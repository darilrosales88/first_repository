<template>
  <div class="form-container">
    <h2>Editar embarcación {{ embarcacion.nombre_embarcacion }}</h2>
    <form @submit.prevent="submitForm">
      <div class="form-group">
        <label for="nombre"> Nombre</label>
        <input type="text" v-model="embarcacion.nombre_embarcacion" required />
      </div>

      <div class="form-group">
        <label for="nacionalidad">Nacionalidad</label>
        <select v-model="embarcacion.nacionalidad" required>
      ...
      <div class="form-group">
        ...
        <input type="number" v-model="embarcacion.eslora" step="0.01" required />
      </div>

      <div class="form-group">
        <label for="manga">Manga</label>
        <input type="number" v-model="embarcacion.manga" step="0.01" required />
      </div>

      <div class="form-group">
        <label for="calado_maximo">Calado máximo</label>
        <input type="number" v-model="embarcacion.calado_maximo" step="0.01" required />
      </div>

      <div class="form-group">
        <label for="desplazamiento_maximo">Desplazamiento máximo</label>
        <input type="number" v-model="embarcacion.desplazamiento_maximo" step="0.01" required />
      </div>

      <div class="form-group">
        <label for="tipo_embarcacion">Tipo de embarcación:</label>
        <select v-model="embarcacion.tipo_embarcacion" required>
      ...
      <div class="form-group">
        ...
        <select v-model="embarcacion.tipo_buque" required>
      ...
      <div class="form-group">
        ...
        <select v-model="embarcacion.tipo_patana" required>
      ...
      <div class="form-group">
        ...
        <input type="text" v-model="embarcacion.imo" required />
      </div>

      <div class="form-group">
        <label for="potencia">Potencia</label>
        <input type="number" v-model="embarcacion.potencia" step="0.01" required />
      </div>
...
export default {
  ...
  data() {
    return {
      embarcacion: {},
      nationalities: [], // Arreglo de los objetos del nomenclador de país
  ...
  methods: {
    ...
    async get_embarcacion() {
      this.$store.commit('setIsLoading', true);
      const embarcacion_id = this.$route.params.id;
      try {
        const response = await axios.get(`/api/embarcaciones/${embarcacion_id}/`);
        this.embarcacion = response.data;
      } catch (error) {
        console.error('Error al obtener la embarcación:', error);
        Swal.fire('Error', 'No se pudo cargar la embarcación.', 'error');
      }
    ...
    async submitForm() {
      ...
      try {
        const data = {
          ...this.embarcacion,
          nacionalidad: this.embarcacion.nacionalidad,
        };

        await axios.patch(`/api/embarcaciones/${embarcacion_id}/`, data);
        Swal.fire('Actualizado!', 'La embarcación ha sido modificada exitosamente.', 'success');
        this.$router.push('/Embarcaciones');
      } catch (error) {
        console.error('Error al actualizar la embarcación:', error);
        Swal.fire('Error', 'Hubo un error al actualizar la embarcación.', 'error');
      } finally {
        this.$store.commit('setIsLoading', false);
      }
    },
