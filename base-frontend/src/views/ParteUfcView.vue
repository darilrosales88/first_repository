...
export default {
  ...
  methods: {
    ...
    async search_embarcacion() {
      ...
      try {
        const response = await axios.get(
          `/api/embarcaciones/?nombre_embarcacion=${this.searchQuery}`
        );
        this.embarcaciones = response.data;
      ...
      <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 10px; width: 100%;">
        ...
        <div>
          ...
          <select id="nacionalidad" class="swal2-select" required style="width: 90%; padding: 5px; font-size: 14px;">
            ${this.nationalities
              .map((n) => `<option value="${n.id}">${n.nombre_pais}</option>`)
              .join("")}
          </select>
        </div>
        <div>
    ...
    `,
        ...
        cancelButtonText: "Cancelar",
        preConfirm: () => {
          const nombre_embarcacion =
            document.getElementById("nombre_embarcacion").value;
          ...
          const calado_maximo = parseFloat(
            document.getElementById("calado_maximo").value
          );
          const desplazamiento_maximo = parseFloat(
            document.getElementById("desplazamiento_maximo").value
          );
          const tipo_embarcacion =
            document.getElementById("tipo_embarcacion").value;
          ...
          if (imo && !/^[A-Za-z]{3}[0-9]{7}$/.test(imo)) {
            Swal.showValidationMessage(
              "El IMO debe comenzar con 3 letras seguidas de 7 dígitos."
            );
            return false;
          }
      ...
      if (formValues) {
        try {
          ...
          Swal.fire(
            "Éxito",
            "La embarcación ha sido añadida exitosamente.",
            "success"
          );
          ...
          if (error.response && error.response.data) {
            Swal.fire(
              "Error",
              `Hubo un error al agregar la embarcación: ${JSON.stringify(
                error.response.data
              )}`,
              "error"
            );
            ...
            Swal.fire(
              "Error",
              "Hubo un error al agregar la embarcación.",
              "error"
            );
          }
        }
