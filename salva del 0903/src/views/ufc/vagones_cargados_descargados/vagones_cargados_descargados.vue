<template>
    <div>
      ...
      <div class="search-container">
        <form class="d-flex search-form" @submit.prevent="searchVagon">
          <input
      ...
      <div class="create-button-container">
        <router-link v-if="hasGroup('Admin')" class="create-button" to="/adicionar_vagon_cargado_descargado">
          Crear vagón <i class="bi bi-plus-circle"></i>
        </router-link> 
      </div>
      <br>
      ...
      <div class="table-responsive">
        <table class="table">
          ...
          <tbody>
            ...
            <td>{{ item.origen }}</td>
            <td>{{ item.tipo_equipo_ferroviario_name }}</td>
            <td>{{ item.estado_name }}</td>
            <td>{{ item.operacion }}</td>
            <td>{{ item.plan_diario_carga_descarga }}</td>
            <td>{{ item.real_carga_descarga }}</td>
            <td>{{ item.tipo_destino_name }}</td> <!-- Aquí se muestra el tipo de destino -->
            <td>{{ item.destino }}</td>
    ...
    methods: {
      ...
      async deleteVagon(id) {
        try {
          ...
          Swal.fire('Eliminado!', 'El vagón ha sido eliminado exitosamente.', 'success');
        } catch (error) {
          console.error('Error al eliminar el vagón:', error);
          Swal.fire('Error', 'Hubo un error al eliminar el vagón.', 'error');
        }
      },
    },
  };
