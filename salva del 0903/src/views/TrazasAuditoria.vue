<template>
  <div>
    ...
    <form class="d-flex" @submit.prevent="handleSearch" style="padding: 10px;">
      <label for="searchQuery" style="padding-right: 10px;">Buscar por usuario:</label>
      <input id="searchQuery" class="form-control form-control-sm me-2" type="search" placeholder="Buscar por usuario..." aria-label="Search" v-model="searchQuery" @input="handleSearchInput" style="width: 200px;">
      
      <!-- Campos de filtrado por rango de fechas -->
      <label for="filterFechaInicio" style="padding-right: 10px;">Fecha inicio:</label>
      <input id="filterFechaInicio" type="date" class="form-control form-control-sm me-2" v-model="filterFechaInicio" @input="handleSearchInput" placeholder="Fecha inicio">
      
      <label for="filterFechaFin" style="padding-right: 10px;">Fecha fin:</label>
      <input id="filterFechaFin" type="date" class="form-control form-control-sm me-2" v-model="filterFechaFin" @input="handleSearchInput" placeholder="Fecha fin">
      
      <!-- Campo de filtrado por acción -->
      <label for="filterAccion" style="padding-right: 10px;">Acción:</label>
      <input id="filterAccion" type="text" class="form-control form-control-sm me-2" v-model="filterAccion" @input="handleSearchInput" placeholder="Acción">
      ...
      <table class="table">
        ...
        <tbody>
          <tr v-for="(item, index) in auditorias" :key="item.id">
            <th scope="row" style="background-color: white;">{{ (currentPage - 1) * itemsPerPage + index + 1 }}</th>
...
export default {
  ...
  methods: {
    ...
    async fetchAuditorias() {
      try {
        const fechaInicio = this.filterFechaInicio ? `${this.filterFechaInicio}T00:00:00` : null;
        const fechaFin = this.filterFechaFin ? `${this.filterFechaFin}T23:59:59` : null;
