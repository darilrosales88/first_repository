      ...
      <div class="card border">
        ...
        <div class="card-body p-3">
          <div class="d-flex justify-content-between align-items-center mb-4">
            <router-link v-if="hasGroup('Admin')" to="AdicionarPais">
                ...
                <tr v-for="item in paises" :key="item.id">
                  ...
                  <td >
                    <div class="d-flex" style="margin-left: 30%;">
                      <button 
                        @click="openPaisesDetailsModal(item)"
  ...
  methods: {
    ...
    async getpaises() {
      try {
        ...
        this.paises = response.data.results; // Obtener los registros de la página actual
        ...
        this.busqueda_existente = true; // Reinicia la variable al cargar todos los países
      } catch (error) {
    ...
    async deletePaises(id) {
      try {
        ...
        Swal.fire(
          "Eliminado!",
          "El país ha sido eliminado exitosamente.",
          "success"
        );
      } catch (error) {
    ...
    openPaisesDetailsModal(Paises) {
      ...
      const gruposAsignados =
        Paises.groups && Paises.groups.length > 0
          ? Paises.groups
              .map((groupId) => {
      ...
      const permisosAsignados =
        Paises.Paises_permissions && Paises.Paises_permissions.length > 0
          ? Paises.Paises_permissions.map((permisoId) => {
