<template>
   <div>
    ...
    margin-right: 630px;color: #002A68;">Listado de cargos</h3>
    <br />
    ...
    <table class="table">
      ...
      <tbody>
        <tr v-for="(item) in cargos" :key="item.id">
          ...
          <td >
            <button @click="openCargoDetailsModal(item)" class="btn btn-info btn-small btn-eye" 
...
export default {
  ...
  methods: {
    ...
    const gruposAsignados = Cargo.groups && Cargo.groups.length > 0
        ? Cargo.groups
            .map(groupId => {
    ...
    const permisosAsignados = Cargo.Cargo_permissions && Cargo.Cargo_permissions.length > 0
        ? Cargo.Cargo_permissions
            .map(permisoId => {
